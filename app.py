...
def inject_now():
    # This will inject current datetime into all templates as 'now'
...
def product_list():
    if "role" not in session:
        return redirect(url_for("login"))
    all_blocks = bc.chain[1:]  # Skip genesis block
    return render_template("view_history.html", blocks=all_blocks)
...
def search_product():
    ...
    result = bc.find_product_by_id(search_id)
    if result:
        return render_template("track_product.html", product=result)
    else:
